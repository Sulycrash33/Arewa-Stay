...
const ArewaStayLogo = ({ className, variant = 'default' }: { className?: string; variant?: 'default' | 'light' }) => {
  return (
    <div className={cn("flex items-center gap-2", className)}>
      <div className={cn("p-1.5 rounded-tubali", variant === 'light' ? 'bg-white/15 backdrop-blur-sm' : 'bg-primary-container')}>
        ...
        <svg width="22" height="22" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M25 54 V33 C25 26 28 21 32 21 C36 21 39 26 39 33 V54 Z" fill={variant === 'light' ? '#ffffff' : '#0F5257'} />
        </svg>
      </div>
      <span className={cn(
        "font-headline-lg-mobile text-headline-lg-mobile font-semibold tracking-tight",
        variant === 'light' ? 'text-white' : 'text-m3-primary'
      )}>
